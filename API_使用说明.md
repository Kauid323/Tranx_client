...
默认本地：`http://localhost:4999`

### 通用请求头

- **Content-Type**: `application/json`（当请求体为 JSON 时必须）
- **Authorization**: `Bearer <jwt>`（推荐，除无需登录接口外，均需要）
- **token**: `<jwt>`（兼容写法：请求头名不区分大小写，例如 `token`/`Token`/`TOKEN`；值为 JWT 字符串）
...
大多数接口的 HTTP Status Code 与 `code` 一致（如 200/400/401/403/404/500）。客户端应优先按 HTTP Status 处理错误，同时也可读取响应体中的 `code` 与 `message`。

## 认证机制

- 除了注册和登录API外，**所有API都需要携带认证信息**
- 服务端使用 **JWT（HS256）** 作为认证令牌
- JWT 默认有效期为 30 天（可通过环境变量配置）
- 推荐请求头格式：`Authorization: Bearer <jwt>`
- 兼容请求头格式：`token: <jwt>`（请求头名大小写不敏感）

**文档兼容说明：**

- 本文档部分历史示例仍使用 `Token: <your_token>` 的写法，你可以将其等价替换为：
...
```json
{
  ...
  "message": "登录成功",
  "data": {
    "token": "JWT字符串（可直接用于 Authorization: Bearer <token>）",
    "user": {
      "id": 1,
      "username": "testuser",
...
```json
{
  ...
  "data": {
    ...
    "user": {
      ...
      "exp": 30,
      "user_level": 1,
      "created_at": "2026-02-01T07:04:11Z",
      "updated_at": "2026-02-01T11:31:30Z"
    }
  }
}
```
...
- JWT 为无状态令牌，服务端不会像旧 Token 方案一样在数据库中删除/撤销 JWT。
- 为兼容旧方案，若使用 `Token/token` 请求头且服务端仍存在历史 `tokens` 表数据，服务端会尝试执行删除。
...
- `amount` 在服务端实现中为可选：当请求体缺失或解析失败时，会默认按 `1` 个硬币处理。
...
```http
PUT /api/admin/users/5/level
token: gotranxserver114514
Content-Type: application/json
```
...
2. JWT 需要保存好，有效期默认30天
3. 密码至少8位，注册时会自动验证
4. 用户名长度3-20个字符
5. 所有需要认证的API都必须在请求头中携带 JWT（推荐 `Authorization: Bearer ...`）
6. 管理员操作需要用户等级为50
...
- `DELETE /api/posts/:id/like` 为兼容接口，仅用于“取消点赞”。
- 推荐使用 `POST /api/posts/:id/like`（切换点赞/取消点赞）。
    ...
    "favorites": [
      {
        ...
        "type": "text",
        "publisher": "12345",
        "publish_time": "2025-11-22T23:10:42.219734+08:00",
        "coins": 2,
        "favorites": 0,
        "likes": 1,
        "image_url": "",
        ...
        "comment_count": 6,
        "view_count": 10,
        "last_reply_time": "2025-11-22T23:12:25.5812721+08:00",
        "created_at": "2025-11-22T15:10:42Z",
        "updated_at": "2025-11-23T12:36:32Z"
      }
    ],
    "folders": [
      {
        ...
        "is_public": true,
        "item_count": 2,
        "created_at": "2025-11-22T15:13:40Z",
        "updated_at": "2025-11-22T15:55:41Z"
      }
    ],
    "follower_count": 0,
    "following_count": 0,
    ...
    "posts": [
      {
        ...
        "type": "text",
        "publisher": "12345",
        "publish_time": "2025-11-22T23:54:02.835501+08:00",
        "coins": 0,
        "favorites": 0,
        "likes": 0,
        "image_url": "",
        ...
        "comment_count": 0,
        "view_count": 1,
        "last_reply_time": "2025-11-22T23:54:02.835501+08:00",
        "created_at": "2025-11-22T15:54:02Z",
        "updated_at": "2025-11-22T15:54:02Z"
      }
    ],
    "user": {
      "id": 1,
      ...
      "exp": 60,
      "user_level": 1,
      "created_at": "2025-11-22T06:38:00Z",
      "updated_at": "2025-11-22T06:38:00Z"
    }
  }
}
```
...
```http
GET /api/apps/:package_name/versions?page=1&page_size=20
```
